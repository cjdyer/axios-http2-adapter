...
async function isHTTP2Supported(
    ...
    const url: URL = new URL(config.url!, config.baseURL);
    ...
    try {
        const res: http2.ResolveProtocolResult =
            await http2.auto.resolveProtocol({
                host: url.host,
    ...
    const requestWrappedWithRedirects = wrap({
        https: {
            request: (options: http2.ClientRequest, handleResponse) => {
                ...
                const req: http2.ClientRequest = http2.request(
                    options,
                    handleResponse
                );
